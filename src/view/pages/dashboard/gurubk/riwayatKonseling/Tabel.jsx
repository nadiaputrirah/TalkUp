...
export default function RiwayatKonselingTable() {
  ...
  const fetchData = async () => {
    try {
      ...
      const transformedData = response.data.data.map((item) => ({
        ...
        tanggal: item.tgl_selesai || item.tgl_pengajuan,
        ...
        catatanSiswa: item.catatan_siswa || "-",
      }));
  ...
  const handleConfirmDelete = async () => {
    try {
      const token = localStorage.getItem("token");
      await axios.delete(
        `http://40.117.43.104/api/v1/konseling/riwayat/${deleteId}`,
        {
          headers: {
            "Authorization": `Bearer ${token}`,
            "Content-Type": "application/json"
  ...
  return (
    <>
      <CustomTable
        ...
        showAddButton={false}
        emptyMessage="Tidak ada riwayat konseling yang ditemukan"
      />
